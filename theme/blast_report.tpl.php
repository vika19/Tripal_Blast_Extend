...
/**
 * ADDED BY SHERI SANDERS OCT 2016 - Add a link to jbrowse.
 * If only one Jbrowse, then use the default, if more than one species served,
 * will have to link the database to the correct instance.
*/

//Adjust this for multiple browsers - could make this more elegant by including a link between db name
//and the browser folder, but this requires a change to default blast databse maker.  For now, hacked ;).
   
   //ADD JBROWSE LINK HERE
   $jbrowselink = $_SERVER['SERVER_NAME'] . "/jbrowse/index.html?data=";

   //MAP JBROWSE DATABASES HERE
   $jb="data"; //default
   $default = 1;
   $db = $blast_job->blastdb->db_name;
   $handle = fopen("/var/www/html/sites/all/modules/tripal_blast_ext/jbrowse_hash.txt", "r");
   if ($handle) {
        while (($line = fgets($handle)) != false) {
	$entry = explode("\t", $line);
	$hash=array($entry[0] => $entry[1]);
        }
     $jb = $hash[$db];
     $default = 0;
     fclose($handle);
...
//   Former hardlinking:
//   if ($db=="volvox demo - nucleotide")
//	$jb="sample_data/json/volvox";

   $link = convert_tsv2jblink($output_files['tsv']['path']);
   if ($default==0) { 
       print "<strong>Visualization:</strong>" . ' <a href="http://'. $jbrowselink . $jb . $link.'">JBrowse</a> (location will be set to the first hit) NOTE: Only works for 10 or less hits.  For blast results with more hits, please download the GFF3 file and upload with the browser GUI tools.';
   }
?>
<br />
